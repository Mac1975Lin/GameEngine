...
bool PlayModeScene::loadSceneGraphCoreJson(const char *path, const Vector3 &offset)
{
	FileInputStream is(path);
	if(!is.opened())
		return false;

	bool rval = loadEntitiesJson(is, offset);

	return rval;
}

bool PlayModeScene::loadEntitiesJson(InputStream &is, const Vector3 &offset)
{
	Vector<int>			parentIndices;
	Vector<Entity *>	entities;
