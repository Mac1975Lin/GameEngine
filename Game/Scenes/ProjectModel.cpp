...
, compressedLevel(ProjectModelBase::UNCOMPRESSED)
{
...
void ProjectModel::setCompressedLevel(ProjectModel::CompressedLevel compressedLevel_)
{
	compressedLevel = compressedLevel_;
	Stage::setCompressLevel(compressedLevel);

	setChanged(COMPRESSED_LEVEL_CHANGED);
	notifyViews();
}

ProjectModel::CompressedLevel ProjectModel::getCompressedLevel()
{
...
bool ProjectModel::loadSceneGraphCoreJson(const char *path, const Vector3 &offset)
{
	return true;
}

bool ProjectModel::loadEntitiesJson(InputStream &is, const Vector3 &offset)
{
	return true;
}

bool ProjectModel::saveSceneGraphCoreJson(const char *path)
{
	return true;
}

bool ProjectModel::saveEntitiesJson(OutputStream &os, Vector<Entity *> &entities)
{
	return true;
}
