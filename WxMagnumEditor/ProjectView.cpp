...
void ProjectView::initUI()
{
	...
	m_mgr.AddPane(sceneView, 
				  ...
				  BestSize(wxSize(viewWidth, viewHeight)).
				  MinSize(wxSize(viewWidth, viewHeight)).
	...
	wxAuiPaneInfo().Name(wxT("InspectorView")).Caption(Localization::getInspectorViewCreatorsTitle()).
				  BestSize(wxSize(560, 640)).
				  MinSize(wxSize(560, 640)).
	...
	wxAuiPaneInfo().Name(wxT("ResourceImporterView")).Caption(Localization::getResourceImporterViewTitle()).
				  BestSize(wxSize(640, 384)).
				  MinSize(wxSize(640, 384)).
...
void ProjectView::initMenuBar()
{
	...
	miscAMenu->Append(MenuCommand::MENU_MISC_A_TOGGLE_COMPRESS_MODE				, Localization::getMenuMiscToggleCompressMode			()			);	
...
void ProjectView::onMenuSelected(wxCommandEvent &evt)
{
	switch(evt.GetId())
	{
		...
		case MenuCommand::MENU_MISC_A_TOGGLE_COMPRESS_MODE:
	...
	switch(evt.GetId())
	{
		...
		case MenuCommand::MENU_FILE_SCENE_NEW:
		{
			...
			wxFileDialog openDialog(this, wxT("New Scene"), sceneDir, wxEmptyString, 
									wxT("DreamEngine Scene files (*.MagnumScene)|*.MagnumScene"),
		...
		case MenuCommand::MENU_FILE_SCENE_DELETE:
		{
			...
			wxFileDialog openDialog(this, _("Select file to delete"), sceneDir, wxEmptyString, 
									_("DreamEngine Scene files (*.MagnumScene)|*.MagnumScene"),
									wxFD_OPEN | wxFD_FILE_MUST_EXIST, wxDefaultPosition);
		...
		case MenuCommand::MENU_FILE_OPEN_PREFAB:
		{
			...
			wxFileDialog openDialog(this, wxT("Open Prefab"), prefabDir, wxEmptyString, 
									wxT("DreamEngine Prefab files (*.magnumPrefab)|*.magnumPrefab"),
									wxFD_OPEN | wxFD_FILE_MUST_EXIST, wxDefaultPosition);
...
wxString ProjectView::getPath(const char *src)
{
	wxString rval;

	char dst[4096];
	GetCurrentDirectoryA(4096, dst);
	strcat(dst, src);

	rval = fixBackslash(wxString(dst));

	return rval;
}

void ProjectView::buildPlatformWin32()
{
	wxDirDialog dirDialog(this, wxT("Build Directory"), "./", wxFD_SAVE | wxFD_OVERWRITE_PROMPT, wxDefaultPosition);
 
	// Creates a Save Dialog with 4 file types
	if(dirDialog.ShowModal() == wxID_OK) // If the user clicked "OK"
	{
		wxString batchFilePath		= fixBackslash(getPath("/../PlatformBuilder/buildWin32Debug.bat"));
		wxString workingPath		= fixBackslash(getPath("/../PlatformBuilder/"));
		wxString dialogPath			= fixBackslash(dirDialog.GetPath());

		wxString parameter			= wxString("MagnumPCLauncher.exe") + " ";
		parameter					+= "\"" + wxString(dialogPath) + "\"";

		ShellExecuteA(0, "open", batchFilePath, parameter, workingPath, SW_SHOWNORMAL);
		ShellExecuteA(0, "open", dialogPath.c_str(), 0, 0, SW_SHOWNORMAL);
	}
}

void ProjectView::buildPlatformIOS()
{
	wxDirDialog dirDialog(this, wxT("Build Directory"), "./", wxFD_SAVE | wxFD_OVERWRITE_PROMPT, wxDefaultPosition);
	...
	if(dirDialog.ShowModal() == wxID_OK) // If the user clicked "OK"
	{
		wxString batchFilePath		= fixBackslash(getPath("/../PlatformBuilder/buildIOSDebug.bat"));
		wxString workingPath		= fixBackslash(getPath("/../PlatformBuilder/"));
		wxString dialogPath			= fixBackslash(dirDialog.GetPath());
	...
	/*
	wxString batchFilePath		= fixBackslash(getPath("/../PlatformBuilder/buildAndroidDebug.bat"));
	wxString workingPath		= fixBackslash(getPath("/../PlatformBuilder/"));
	wxString androidStudioExe	= fixBackslash("C:/Android/Android Studio/bin/studio64.exe");

	ShellExecuteA(0, "open", batchFilePath, 0, workingPath, SW_SHOWNORMAL);
	*/
}

void ProjectView::buildPlatformAndroid()
{
	wxDirDialog dirDialog(this, wxT("Build Directory"), "./", wxFD_SAVE | wxFD_OVERWRITE_PROMPT, wxDefaultPosition);
...
void ProjectView::buildPlatformWinMobile()
{
}

void ProjectView::buildPlatformPS3()
{
}

void ProjectView::buildPlatformPSP()
{
}

void ProjectView::buildPlatformPSVita()
{
}

void ProjectView::buildPlatformWii()
{
}

void ProjectView::buildPlatformWiiU()
{
}
...
void ProjectView::onMiscAMenuSelected(wxCommandEvent &evt)
{
	...
	switch(evt.GetId())
	{
		...
		break;
		case MenuCommand::MENU_MISC_A_TOGGLE_COMPRESS_MODE:
			os << String("ToggleCompressedMode");
			GameDescriptor::instance().onGUIEvent(&buffer[0], buffer.length());
		break;
		case MenuCommand::MENU_MISC_A_1:
		break;
...
void ProjectView::notify(int flags, ProjectModelBase &projectModel, const Vector<unsigned char> &buffer)
{
	...
	if(flags & ProjectModelBase::STATE_CHANGED)
	{
		...
		{
			menuBar->Enable(MenuCommand::MENU_BUILD_PLATFORM_WIN32			, true);
			menuBar->Enable(MenuCommand::MENU_BUILD_PLATFORM_IOS			, true);
			menuBar->Enable(MenuCommand::MENU_BUILD_PLATFORM_ANDROID		, true);
			//menuBar->Enable(MenuCommand::MENU_BUILD_PLATFORM_WINMOBILE	, true);
			//menuBar->Enable(MenuCommand::MENU_BUILD_PLATFORM_PS3			, true);
			//menuBar->Enable(MenuCommand::MENU_BUILD_PLATFORM_PSP			, true);
			//menuBar->Enable(MenuCommand::MENU_BUILD_PLATFORM_PSVITA		, true);
			//menuBar->Enable(MenuCommand::MENU_BUILD_PLATFORM_WII			, true);
			//menuBar->Enable(MenuCommand::MENU_BUILD_PLATFORM_WIIU			, true);
		}
		...
		if(projectModel.getCurrentState()=="DebugRun")
		{
			...
			menuBar->Enable(MenuCommand::MENU_MISC_A_TOGGLE_COMPRESS_MODE		, false);
		...
		else
		{
			...
			menuBar->Enable(MenuCommand::MENU_MISC_A_TOGGLE_COMPRESS_MODE		, true);
